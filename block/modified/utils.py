...
class Block_Assign:
    def __init__(self, assignment_index, block_split_dict, centers):
        ...
        print("reassign", assignment_index)
        self.block_split_dict = block_split_dict

        self.block2center = dict()
        self.center2block = [[] for _ in centers]
        self.centers = centers
        print(len(self.center2block))
        for i in range(len(self.center2block)):
            print(len(self.center2block[i]))
...
if __name__ == "__main__":
    ...
    with open(f"autodgr/block_out/assignment_hybrid_4.pkl", "rb") as file2:
