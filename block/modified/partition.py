...
from block.blocklize import MODEL_BLOCKS, MODEL_ZOO
from block.utils import Block, Block_Assign, Block_Sim


def parse_args():
    parser = argparse.ArgumentParser(description="mmcls test model")
    parser.add_argument("--sim_path", default="./simlarity/out/sim_cka/")
...
def reassign(args, block_split_dict, block_sims, assignment):
    ...
    for i, center_block in enumerate(centers):
        for m, other_model_name in enumerate(MODEL_ZOO):
            for j, block in enumerate(block_split_dict[other_model_name]):
                # Center block itself has the maximum similarity
                if block == center_block:
                    block_sim = block_sims.get_sim(center_block, block)
                    print("center:", block_sim)
                    block_sim_map[i, m, j] = np.inf  
                else:
                    block_sim = block_sims.get_sim(center_block, block)
                    print("notcen:", block_sim)
    ...
    for center in centers:
        print(center.model_name, "-", center.block_index)
    print(assignment_index)
    assignment = Block_Assign(
...
def repartition(args, block_split_dict, block_sims, all_assignmemt):
    ...
    for m_id, model_name in enumerate(MODEL_ZOO):
        ...
        for b_id in range(len(iter_block_split) - 1):
            ...
            best_cost = compute_cost(block1, all_assignmemt, block_sims) + compute_cost(
                block2, all_assignmemt, block_sims
            )
            concat_nodes = block1.node_list + block2.node_list
            if (
                len2 > block_split_dict["min_node"]
                and len2 <= block_split_dict["max_node"]
            ):
                block1 = Block(model_name, b_id, concat_nodes[: len1 + 1])
                block2 = Block(model_name, b_id + 1, concat_nodes[len1 + 1 :])
                new_cost = compute_cost(
                    block1, all_assignmemt, block_sims
            ...
            if (
                len1 > block_split_dict["min_node"]
                and len1 <= block_split_dict["max_node"]
            ):
                block1 = Block(model_name, b_id, concat_nodes[: len1 - 1])
                block2 = Block(model_name, b_id + 1, concat_nodes[len1 - 1 :])
    ...
    all_assignmemt.update_block_split(block_split_dict)
...
def print_partition(block_split_dict):
    for model_name in MODEL_ZOO:
        print(f"[{model_name}]")
        model_split = ".".join([str(block) for block in block_split_dict[model_name]])
        print(model_split)
...
def main():
    ...
    with open(os.path.join(args.out, f"similarity.pkl"), "wb") as f:
        pkl.dump(block_sims, f)
